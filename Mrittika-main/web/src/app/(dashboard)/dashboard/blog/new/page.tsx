      ...
      <div className="lg:w-[35%] space-y-4 lg:sticky lg:top-8 lg:self-start">
        <div className="rounded-xl border border-brand-sand bg-white p-5 space-y-4">
          ...
          <div>
            ...
            <Input value={form.tags} onChange={(e) => setForm((p) => ({ ...p, tags: e.target.value }))} placeholder="skincare, natural, face pack" className="mt-1" />
          </div>
          <div>
            <Label>Excerpt</Label>
            <Textarea value={form.excerpt} onChange={(e) => setForm((p) => ({ ...p, excerpt: e.target.value }))} placeholder="Brief description..." className="mt-1" rows={2} />
          </div>
          <div>
            <Label>Featured Image URL</Label>
            <Input value={form.featured_image} onChange={(e) => setForm((p) => ({ ...p, featured_image: e.target.value }))} placeholder="https://..." className="mt-1" />
            {form.featured_image && (
